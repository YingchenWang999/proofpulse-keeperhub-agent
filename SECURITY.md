...
12. RPC, Ollama, and KeeperHub child-process operations have finite timeouts.
...
15. A live KeeperHub result is accepted only when status is `completed` and both the execution ID and transaction hash are present.
...
- KeeperHub credentials can authorize actions outside this repository. Store them only in the OS keychain or environment and use a dedicated least-privilege organisation.
